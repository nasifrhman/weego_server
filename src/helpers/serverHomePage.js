  ...
  errorLogs.forEach(entry => {
    tableHtml += `
      <tr>
        ...
        <td>${entry.message}</td>
        <td><pre class="log-trace">${entry.errorPath}</pre></td>
        <td><pre class="stack-trace">${entry.stack}</pre></td>
      </tr>
    `;
  });
...
function generateResponseTimeTable(responseTimeLogs) {
  ...
  responseTimeLogs.forEach(entry => {
    ...
    tableHtml += `
      <tr class="${labelClass}">
        <td>${entry.timestamp}</td>
        <td><pre class="log-trace">${entry.route}</pre></td>
        <td>${entry.method}</td>
...
// Function to generate CPU usage HTML
function generateCpuUsageHtml() {
...
// Function to generate the HTML page with authentication
